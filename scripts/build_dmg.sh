...
func drawPanel(_ rect: NSRect) {
    ...
    NSColor(calibratedRed: 0.72, green: 0.82, blue: 0.90, alpha: 1).setStroke()
    panel.lineWidth = 2
    panel.stroke()
}
...
NSColor(calibratedRed: 0.14, green: 0.45, blue: 0.72, alpha: 1).setStroke()
arrowPath.stroke()
...
head.close()
NSColor(calibratedRed: 0.14, green: 0.45, blue: 0.72, alpha: 1).setFill()
head.fill()

drawText(
    "Drag ProfileSmith to Applications",
    ...
    color: NSColor(calibratedRed: 0.08, green: 0.18, blue: 0.28, alpha: 1)
)
...
sign_nested_target() {
    ...
    entitlements_file="$(mktemp "${TMPDIR:-/tmp}/profilesmith-nested-entitlements.XXXXXX")"
...
resign_for_distribution() {
    ...
    entitlements_file="$(mktemp "${TMPDIR:-/tmp}/profilesmith-entitlements.XXXXXX")"
