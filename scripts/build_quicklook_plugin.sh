...
needs_rebuild=0
if [[ ! -d "$PREVIEW_CACHE_PATH" ]]; then
    needs_rebuild=1
elif [[ "$SCRIPT_DIR/build_quicklook_plugin.sh" -nt "$PREVIEW_CACHE_PATH" ]]; then
    needs_rebuild=1
elif find "$PLUGIN_ROOT" -type f \
    ! -path '*/project.xcworkspace/*' \
    ! -path '*/xcuserdata/*' \
    -newer "$PREVIEW_CACHE_PATH" -print -quit | grep -q .; then
    needs_rebuild=1
fi

if [[ "$needs_rebuild" -eq 1 ]]; then
    rm -rf "$DERIVED_DATA_PATH" "$PREVIEW_CACHE_PATH"

    echo "Building ProfileSmithQuickLookPreview for $BUILD_CONFIGURATION"
    /usr/bin/env -i \
        HOME="$HOME" \
        PATH="$PATH" \
        TMPDIR="${TMPDIR:-/tmp}" \
        ...
        -scheme "ProfileSmithQuickLookPreview" \
...
rm -rf "$PLUGINS_OUTPUT_DIR/$PREVIEW_PRODUCT_NAME"
